...
import controllers.FillController;
...
public class Server {
    ...
    private void run(String portNumber) {
        ...
        // Create and install the HTTP handler for the following URL paths.
        // When the HttpServer receives an HTTP request containing the
        // URL path, it will forward the request to correct controller
        // for processing.
        server.createContext("/user/register", new RegisterController());
        server.createContext("/", new FileController());
        server.createContext("/fill", new FillController());
        server.createContext("/clear", new ClearController());
        server.createContext("/user/login", new LoginController());
        server.createContext("/person", new PersonController());
        server.createContext("/load", new LoadController());
        server.createContext("/event", new EventController());
